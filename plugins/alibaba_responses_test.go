...
func TestAlibabaResponsesProtocol(t *testing.T) {
	...
	t.Run("keeps unknown task statuses pollable", func(t *testing.T) {
		value, callErr := plugin.Engine.Call(
			t.Context(),
			"parseTaskResult",
			map[string]any{},
			map[string]any{"output": map[string]any{"task_status": "UNKNOWN"}},
		)
		require.NoError(t, callErr)
		encoded, marshalErr := common.Marshal(value)
		require.NoError(t, marshalErr)
	...
	t.Run("maps explicit task failure statuses to failure", func(t *testing.T) {
		for _, status := range []string{"FAILED", "ERROR", "CANCELED", "REJECTED", "EXPIRED"} {
			t.Run(status, func(t *testing.T) {
				value, callErr := plugin.Engine.Call(
					...
					"parseTaskResult",
					map[string]any{},
					map[string]any{"output": map[string]any{"task_status": status}},
				)
				require.NoError(t, callErr)
				encoded, marshalErr := common.Marshal(value)
				require.NoError(t, marshalErr)
